...
assert.ok(script.story_bible);
assert.ok(Array.isArray(script.foreshadowing_ledger));
assert.ok(Array.isArray(script.canon_facts));
assert.ok(script.rhythm_plan);
assert.ok(script.coverage_report);
assert.ok(script.scenes[0].visualization_checks);
assert.ok(Array.isArray(script.scenes[0].beats[0].source_refs));
